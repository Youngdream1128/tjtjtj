...
extern pthread_mutex_t GoReissueLock;
	...
	while(1)
	{
		if(m_ConnectedState == true && tboxInfo.operateionStatus.isGoToSleep == 1)
		{
			...
			else if(m_loginState == 2)
			{
				...
				#if 0
				if(NowReissueReportTime - ReissueReportTime >= REISSUE_TIME)
				{
					pthread_mutex_lock(&GoReissueLock);
					VehicleReissueSend();
					pthread_mutex_unlock(&GoReissueLock);
					time(&ReissueReportTime);
				}
				#endif
...
#elif DEV_NO == 9
	memcpy(p_FAWACPInfo_Handle->Vin, "LFPZ3ACC000000005", strlen("LFPZ3ACC000000005"));
#endif
#if 1
	//生产时版本打开
...
uint16_t CFAWACP::UnpackData_AcpRemoteCtrl(uint8_t *PayloadBuff, uint16_t payload_size, uint8_t MsgType)
{
	...
	uint8_t	SubitemCodeParam[16] = {0};//子项编码参数值
	//时间戳部分
	ShowdataTime(pos);
