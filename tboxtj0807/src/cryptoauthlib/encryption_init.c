...
static void encryptstring(uint8_t *plaintext,uint16_t keyslot);
static void decryptstring(uint8_t *ciphertext,uint16_t keyslot);
...
static void encryptstring_testing(void)
{
	...
	keyslot = 0;
	encryptstring(&plaintext,keyslot);
}
...
static void encryptstring(uint8_t *plaintext,uint16_t keyslot)
{
	...
	atcab_aes_encrypt(key_slot, 0, &plaintext[data_block * AES_DATA_SIZE], encrypted_data_out);
	for (i = 0; i < sizeof(encrypted_data_out); i++)
    {
        printf("%02X", (int)encrypted_data_out[i]);
    }
    printf("\r\n");
...
static void decryptstring_testing(void)
{
	...
	keyslot = 0;
	decryptstring(&ciphertext,keyslot);
}
static void decryptstring(uint8_t *ciphertext,uint16_t keyslot)
{
	ATCA_STATUS status;
...
int encryptTbox_Data(uint8_t *plaintext, uint16_t keyslot)
{
	uint8_t plaintext_[16]= {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a};;

	encryptstring(&plaintext_, keyslot);
	return 0;
}
//解密接口0:Skey解密 1:Rootkey解密
int decryptTbox_Data(uint8_t *plaintext, uint16_t keyslot)
{
	uint8_t ciphertext[16]= {0x47, 0xC5, 0x8D, 0x5E, 0x21, 0xCA, 0xAF, 0x84, 0x0D, 0x01, 0x5B, 0x7D, 0x9B, 0x91, 0x09, 0x81};
