...
extern int writeTsp_rootkey(void);
//写入skey,可修改但不能读取 slot 0
extern int writeTsp_skey(void);
//加密接口0:Skey加密 1:Rootkey加密	
extern int encryptTbox_Data(uint8_t *plaintext, uint16_t keyslot);
//解密接口0:Skey解密 1:Rootkey解密
extern int decryptTbox_Data(uint8_t *plaintext, uint16_t keyslot);
