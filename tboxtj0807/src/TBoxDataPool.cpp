...
int TBoxDataPool::initPara(void)
{
	...
	//WIFI记忆状态初始化
	uint8_t  wifi_RemindStatus = 0;//默认关闭
	setPara(WIFI_REMINDSTATUS_TD, &wifi_RemindStatus, sizeof(wifi_RemindStatus));
...
int TBoxDataPool::setPara(TBoxDataId id, void *pPara, uint32_t size)
{
	switch (id)
	{
		...
		case OTA_IP_PORT_ID:
			...
			break;
		case WIFI_REMINDSTATUS_TD://WIFI记忆状态
			TBoxPara.dataPara.wifiRemindStatus = *(uint8_t *)pPara;
			break;
...
int TBoxDataPool::getPara(TBoxDataId id, void *pPara, uint32_t size)
{
	switch (id)
	{
		...
		case OTA_IP_PORT_ID:
			...
			break;
		case WIFI_REMINDSTATUS_TD://WIFI记忆状态
			*(uint8_t *)pPara = TBoxPara.dataPara.wifiRemindStatus;		
			break;
		// For TBoxDetectionTime
		case LteModuleDetectionTime_ID:
