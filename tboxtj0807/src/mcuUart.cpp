...
//	MCULOG("frontfoglights == %d\n",p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.frontfoglights );
	...
	if(!PowerONStatus)
	{
		if(CFAWACP::cfawacp->m_loginState == 2 && p_FAWACPInfo_Handle->VehicleCondData.Powersupplymode == 4 && AccStatus == 1){
			reportEventCmd(CFAWACP::cfawacp->timingReportingData, 4, ACPApp_VehicleCondUploadID);//上报车辆上电事件
			PowerONStatus++;
		}
	}else{
		if(CFAWACP::cfawacp->m_loginState == 2 && p_FAWACPInfo_Handle->VehicleCondData.Powersupplymode == 0 && AccStatus == 0){
	...
	if(VehCollideAlarm != SigEvent.s_VehCollideAlarm){
		if(VehCollideAlarm != 0 && CFAWACP::cfawacp->m_loginState == 2)
	...
	switch(SubitemCode)
	{
		...
		case MCUAir_Control:
			...
			for(int i = 0; i< SubitemCodeParam; i++)
			{
				...
				{
					case 0x01:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Control.BitType = *pos++;
			break;
					case 0x02:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_CompressorSwitch.BitType = *pos++;
					break;
					case 0x03:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Temperature.BitType = *pos++;
					break;
					case 0x04:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_SetAirVolume.BitType = *pos++;
			break;
					case 0x05:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_FrontDefrostSwitch.BitType = *pos++;
			break;
					case 0x06:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Heatedrear.BitType = *pos++;
			break;
					case 0x07:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_BlowingMode.BitType = *pos++;
			break;
					case 0x08:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_InOutCirculate.BitType = *pos++;
			break;
					case 0x09:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_AutoSwitch.BitType = *pos++;
			break;
					default:
			break;
				}
...
int mcuUart::cb_RemoteCtrlCmd()
{
	...
	*pos++ = 0;
	...
	{
		printf("SubitemCode == %d\n",CFAWACP::cfawacp->m_AcpRemoteCtrlList.SubitemCode[i]);
		printf("SubitemValue == %d\n",CFAWACP::cfawacp->m_AcpRemoteCtrlList.SubitemValue[i]);
			...
			*pos++ = 0x11;
			...
			case VehicleBody_LuggageCarID:
				*pos++ = 0x17;
				if(!CFAWACP::cfawacp->m_AcpRemoteCtrlList.SubitemValue[i])
		{
					CFAWACP::cfawacp->m_AcpRemoteCtrlList.SubitemValue[i] = 1;
				}else
				{
			CFAWACP::cfawacp->m_AcpRemoteCtrlList.SubitemTotal = 1;
			CFAWACP::cfawacp->m_AcpRemoteCtrlList.SubitemCode[0] = 255;
