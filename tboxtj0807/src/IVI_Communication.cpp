...
uint8_t IVI_Communication::TBOX_WIFI_Info_Query(uint8_t *pData, int len, uint16_t msgType)
{
	...
	{
		setWifiState = true;
		int wifModifyFlag = -1;//WIFI当前是否需要修改用户名和密码标志 0:不需要修改，1:需要修改，-1:无效
		for(i = 0; i<len; i++)
		{
			...
			if(pData[0] == 0)//关闭
			{
				...
				if(tboxInfo.operateionStatus.wifiStartStatus == 0)
				{
					...
					if(ret == 0)
					{
						...
						wifModifyFlag = 0;//不需要修改用户名和密码
					}
				}
			}
			else
			{
				wifModifyFlag = 1;//需要修改用户名和密码
			}
        }
		else
		{
