...
#define LIB_CACULATE_PATH "libnts_crypt.so"
...
int isConfigATECC608A()
{
	uint8_t ATECC608A_Cfg_Status;

	dataPool->getTboxConfigInfo(encryptionID, &ATECC608A_Cfg_Status, 1);
	if(ATECC608A_Cfg_Status == 0)
	{
		write_config();
		lock_config();
		ATECC608A_Cfg_Status = 1;
		dataPool->setTboxConfigInfo(encryptionID, &ATECC608A_Cfg_Status, 1);
	}
