	...
	while(1)
	{
		if(m_ConnectedState == true)
		{
			...
			else if(m_loginState == 2)
			{
				...
				if(flag == 0)
				{
					HighReportTime = NowHighReportTime;
					LowReportTime = NowLowReportTime;
					flag = 1;
				}
				if(NowHighReportTime - HighReportTime >= p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeGpsSpeedInterval)
				{  
					cfawacp->timingReportingData(MESSAGE_TYPE_START + 1, ACPApp_VehicleCondUploadID);
					time(&HighReportTime);
        		}
				if(NowLowReportTime - LowReportTime >= p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeInterval)
				{
...
void *CFAWACP::TimeOutThread(void *arg)
{
	...
	{	
		...
		if(pCFAWACP->m_ConnectedState == true && pCFAWACP->m_loginState == 2)
		{
			...
			sleep(1);
			printf("p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].EmergedCall == %s\n", p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].EmergedCall);
		}
	}
#endif
	pthread_exit(0);
...
void CFAWACP::Init_FAWACP_dataInfo()
{
	...
	memcpy(VerTboxOS, FILE_VERSION, strlen(FILE_VERSION));//FILE_VERSION
...
void CFAWACP::InitAcpTBoxAuthData()
{
...
#elif DEV_NO == 5//天津一汽装车
	memcpy(p_FAWACPInfo_Handle->Vin, "LMGFE3G88D1000602", strlen("LMGFE3G88D1000602"));
	memcpy(p_FAWACPInfo_Handle->ICCID, "89860918700300514814", strlen("89860918700300514814"));
	memcpy(p_FAWACPInfo_Handle->IMEI, "861477035994290", strlen("861477035994290"));
	memcpy(p_FAWACPInfo_Handle->TBoxSerialNumber, "D2061812280A431760000000000005", strlen("D2061812280A431760000000000005"));
	memcpy(p_FAWACPInfo_Handle->IVISerialNumber, "000000000000000000000000000005", strlen("000000000000000000000000000005"));
	...
	printf("====================================================================\n");
	for(uint8_t k = 0; k<sizeof(p_FAWACPInfo_Handle->Vin); k++)
		printf("%02x ", p_FAWACPInfo_Handle->Vin[k]);
	printf("\n\n\n");
...
void CFAWACP::TimeStampPart(uint8_t* pTemp, TimeStamp_t *pTimeStamp)
{
	if(NULL != pTemp && NULL != pTimeStamp)
	{
		...
		pTimeStamp->Seconds = Time.tv_usec / 1000; //毫秒
...
uint16_t CFAWACP::addRemainedOil(uint8_t* pTemp)
{
	...
	*pos++ = p_FAWACPInfo_Handle->VehicleCondData.RemainedOil.RemainedOilValue;
	//*pos++ = p_FAWACPInfo_Handle->VehicleCondData.RemainedOil.RemainedOilGrade; //ACP最新版本打开
			...
			for(uint16_t i = 0; i < m_AcpRemoteCtrlList.SubitemTotal; i++)
			{
				...
				switch(m_AcpRemoteCtrlList.SubitemCode[i])
				{
					...
					case VehicleChargeMode_AppointmentID:
						...
						break;
					case VehicleChargeMode_EmergencyCharg:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleCharge.VehicleCharge_EmergenCharg = *pos++;
						FAWACPLOG("VehicleCharge_EmergenCharg == %d",SubitemCodeParam);
						break;
					case VehicleWIFIStatusID:
