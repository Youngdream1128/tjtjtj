...
int main(int argc,char* argv[]) 
{
	...
	time_t BeginTimeLoop2s, BeginTimeLoop5s, Loop2sTime, Loop5sTime; //数据上报时间间隔
	BeginTimeLoop2s   = time(NULL);
	BeginTimeLoop5s   = time(NULL);
	Loop2sTime  = time(NULL);
	Loop5sTime  = time(NULL);
	...
	{	
		...
		if(cost_tvtime == 40)//40ms计量一次
		{
			...
			{
				printf("iInitCheckPWM Current nVoltage_SRS:%d mv\n", nVoltage_SRS);
				if(SRS_Voltage < 400)//低电平					
					nCheckLowVol++;
				else if(nVoltage_SRS < 1800 && nVoltage_SRS > 1200)//高电平
					nCheckHighVol++;
			}
			gettimeofday(&tvBeginCheckPWM, NULL);
		}
		if(iInitCheckPWM == 0)//初始化状态
		{
			//判断PWM
			if( (nCheckHighVol == 5 && nCheckLowVol == 0) || (nCheckHighVol == 0 && nCheckLowVol == 5) )			
				iInitCheckPWM = 1;
		}	
		elses
		{
			if(nCheckHighVol == 1 && nCheckLowVol == 5)//碰撞周期
			{
				nPeriodCheckPWM++;
			}
			if(nPeriodCheckPWM >= 20)
			{
				printf("^^^^^^^Deployment Cammand SRS ECall nPeriodCheckPWM = %d ^^^^^^^\n",nPeriodCheckPWM);
			}
		}
	
		//count++;
		//if(count%2 == 0)
		if(difftime(Loop2sTime, BeginTimeLoop2s) == 2)
		{
		...
		//else if(count%5 == 0)
		if(difftime(Loop5sTime, BeginTimeLoop5s) == 5)
		{
			datacall_info_type datacall_info;
			get_datacall_info(&datacall_info);
			if(datacall_info.status == DATACALL_DISCONNECTED)
