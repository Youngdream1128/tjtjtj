...
CFAWACP::CFAWACP()
{
	...
	m_SendMode = 0;			//发送模式	0:即时发送	1:补发
	m_bEnableSendDataFlag = true;   //发送使能标志
	cfawacp = this;			//全局对象指针赋值
	Headerlink = NULL;

	if(readTBoxParameterInfo() == -1)	//初始化TBOX信息
	{
		FAWACPLOG("readTBoxParameterInfo is fail\n");
	...
	if(pthread_create(&HighReportThreadId, NULL, HighReportThread, this) != 0)
		FAWACPLOG("Cannot creat HighReportThreadId:%s\n", strerror(errno));
...
//保持连接-
	while(1)
	{
		if(m_ConnectedState == true)
		{
			if(m_loginState == 0)
			{
					FAWACPLOG("m_loginState = %d\n",m_loginState);
					InitAcpTBoxAuthData();
					sendLoginAuthApplyMsg();
					sleep(10);
			}
			else if(m_loginState == 1)
			{
				sleep(1);
			}
			else if(m_loginState == 2)
			{
				FAWACPLOG("NormalReport.dataLen:AppID = %d, MsgID = %d\n",ACPApp_VehicleCondUploadID,(MESSAGE_TYPE_START+2));
				//低频上报普通车况 MsgType == 2
				cfawacp->timingReportingData(MESSAGE_TYPE_START + 2, ACPApp_VehicleCondUploadID);
				sleep(p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeInterval);
			}
		}
		else
		{
...
void *CFAWACP::HighReportThread(void *arg)
{
	...
	while(1)
	{
		...
		{
			FAWACPLOG("HighReport.dataLen:AppID = %d, MsgID = %d\n",ACPApp_VehicleCondUploadID,(MESSAGE_TYPE_START+1));
			//高频上报 ---车速与位置状态	MsgTYPE == 1
			pCFAWACP->timingReportingData(MESSAGE_TYPE_START + 1, ACPApp_VehicleCondUploadID);
			sleep(p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeGpsSpeedInterval);
		}
	}
	pthread_exit(0);
}
...
//等待获取内网IP
	...
	if((m_Socketfd = socket(AF_INET, SOCK_STREAM, 0)) != -1)
	{
		...
		if (ret_socketConnect != -1)
		{
			...
			FAWACPLOG("Current date:%0d-%d-%d",p_tm->tm_year+1900, p_tm->tm_mon, p_tm->tm_mday);
			FAWACPLOG("SYS TIME:%0d-%d-%d",p_tm->tm_year, p_tm->tm_mon,p_tm->tm_mday);

			fd = open(ACPPARAMETER_INFO_PATH, O_RDWR | O_CREAT, 777);
			if (-1 == fd)
			{
...
int CFAWACP::initTBoxParameterInfo()
{
	...
	fd = open(ACPPARAMETER_INFO_PATH, O_RDWR | O_CREAT, 0666);
	if (-1 == fd)
	{
		FAWACPLOG("open file failed\n");
		return -1;
	}
...
void CFAWACP::Init_FAWACP_dataInfo()
{
...
#elif DEV_NO == 4	
	memcpy(p_FAWACPInfo_Handle->Vin, "LMGFE3G88D1000602", strlen("LMGFE3G88D1000602"));
	memcpy(p_FAWACPInfo_Handle->ICCID, "89860000000000000001", strlen("89860000000000000001"));
	memcpy(p_FAWACPInfo_Handle->IMEI, "861477035453860", strlen("861477035453860"));
	memcpy(p_FAWACPInfo_Handle->TBoxSerialNumber, "D2061812280A431760000000000004", strlen("D2061812280A431760000000000004"));
	memcpy(p_FAWACPInfo_Handle->IVISerialNumber, "000000000000000000000000000004", strlen("000000000000000000000000000004"));
#elif DEV_NO == 5	
		memcpy(p_FAWACPInfo_Handle->Vin, "AAAAA3G88D1000602", strlen("AAAAA3G88D1000602"));
		memcpy(p_FAWACPInfo_Handle->ICCID, "55550000000000000001", strlen("55550000000000000001"));
		memcpy(p_FAWACPInfo_Handle->IMEI, "555577035453860", strlen("555577035453860"));
		memcpy(p_FAWACPInfo_Handle->TBoxSerialNumber, "55551812280A431760000000000005", strlen("55551812280A431760000000000005"));
		memcpy(p_FAWACPInfo_Handle->IVISerialNumber, "000000000000000000000000005555", strlen("000000000000000000000000005555"));
	...
	memcpy(p_FAWACPInfo_Handle->New_RootKey, pRootKey, sizeof(p_FAWACPInfo_Handle->New_RootKey));
...
//	p_FAWACPInfo_Handle->CollectTime.CollectPacketSN = 0;
	p_FAWACPInfo_Handle->CollectTime.Year = p_tm->tm_year-90;//这里有函数处理
	p_FAWACPInfo_Handle->CollectTime.Month = p_tm->tm_mon+1;//月，日，时，分，秒，分会变动
	p_FAWACPInfo_Handle->CollectTime.Day = p_tm->tm_mday;
	p_FAWACPInfo_Handle->CollectTime.Hour = p_tm->tm_hour;
	p_FAWACPInfo_Handle->CollectTime.Minutes = p_tm->tm_min;
	p_FAWACPInfo_Handle->CollectTime.Seconds = p_tm->tm_sec;
	p_FAWACPInfo_Handle->CollectTime.msSeconds = Time.tv_usec / 1000; //毫秒
	...
	p_FAWACPInfo_Handle->VehicleCondData.RemainedOil = 0x7F;//剩余油量0x7F：初始值 0x7F：无效值 
	...
	p_FAWACPInfo_Handle->VehicleCondData.LTAverageSpeed = 0x7FFE;//长时平均车速0x7FFE：初始值 0x7FFF：无效值
	p_FAWACPInfo_Handle->VehicleCondData.STAverageSpeed = 0x7FFE;//短时平均车速0x7FFE：初始值 0x7FFF：无效值
	...
	p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.accelerateTime = 0xFE;//急加速持续时间
	//动力电池状态数据
	...
	p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ChargeSchedule.DayAppointment = 0;//预约充电日1-7—>周一到周日
	p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ChargeSchedule.StartTime = 0;//充电开始时间,单位分钟
	p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ChargeSchedule.KeepTime = 0;//持续时间,单位分钟
	...
	p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[1].ChargeSchedule.DayAppointment = 0;//预约充电日1-7—>周一到周日
	p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[1].ChargeSchedule.StartTime = 0;//充电开始时间,单位分钟
	p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[1].ChargeSchedule.KeepTime = 0;//持续时间,单位分钟
	//故障信息数据
	...
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTTSROverSpeedAlarmState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTTSRLimitSpeedState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTAEBInterventionState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTABSInterventionState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTASRInterventionState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTESPInterventionState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTDSMAlarmState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTTowHandOffDiskState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTACCState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTACCSetSpeedState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTFCWAlarmState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTFCWState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTFCWAlarmAccePedalFallState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTFCWAlarmFirstBrakeState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTSLDWState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTLDWAlarmState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTLDWAlarmDireDiskResponseState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTLKAState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTLKAInterventionState = 0x0;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTLKADriverTakeOverPromptState = 0x0;
...
//	AcpTBox_AuthReadyMsg.AuthReadyMsg_VehicleDescriptor.RawData = 0;
...
int	CFAWACP::sendLoginAuth(uint8_t MsgType, AcpCryptFlag_E CryptFlag)
{
	...
	if(false == m_bEnableSendDataFlag)
		return -1;
	uint16_t dataLen;
	...
	FAWACPLOG("Send data dataLen=%d AppId=%d,MsgType=%d,CryptFlag=%d\n",dataLen,ACPApp_AuthenID,MsgType,CryptFlag);
...
{
	int iRet = -1;
	iRet = sendLoginAuth(MESSAGE_TYPE_START+1, AcpCryptFlag_NULL);
	if(iRet == -1)
	{	
		close(m_Socketfd);
		m_loginState = 0;
...
int	CFAWACP::sendLoginCTMsg()
{
	...
	//生成随机数Rand2_Tbox
	HMACMd5_digest((const char *)p_FAWACPInfo_Handle->RootKey, AcpTBox_CTMsg.Rand1CT_TBox, sizeof(AcpTBox_CTMsg.Rand1CT_TBox), AcpTBox_CTMsg.Rand2_Tbox);
...
int	CFAWACP::sendLoginRSMsg()
{
	...
	HMACMd5_digest((const char *)p_FAWACPInfo_Handle->RootKey, Rand2CT_TBox, sizeof(Rand2CT_TBox), AcpTBox_RSMsg.Rand3_TBox);
...
int	CFAWACP::sendLoginAuthReadyMsg()
{
	...
	/*FAWACPLOG("SKey_TBox:");
	for(uint8_t i = 0; i < sizeof(SKey_TBox); i++)
		printf("%02x ", SKey_TBox[i]);
	printf("\n\n");*/
...
//PayloadBuff 执行了加密后的数据
	switch (applicationID)
	{
		...
		case ACPApp_RemoteCtrlID://远程控制
			...
			ErrorCodeDeal(ElementBuff + DATA_TIMESTAMP_SIZE + DATA_AUTHTOKEN_SIZE + DATA_SOURCE_SIZE, 0);//错误码暂时为0
		...
		case ACPApp_QueryVehicleCondID://车况查询
			...
			//全部信号数据
			dataLen = ReportVehicleCondDeal(ElementBuff + DATA_TIMESTAMP_SIZE + DATA_AUTHTOKEN_SIZE + DATA_SOURCE_SIZE + DATA_ERRORCODE_SIZE, 1);
			dataLen += DATA_TIMESTAMP_SIZE + DATA_AUTHTOKEN_SIZE + DATA_SOURCE_SIZE + DATA_ERRORCODE_SIZE;
		...
		case ACPApp_VehicleCondUploadID://车况数据上报 1)事件触发上报 2)普通车况10S定时上报 3) 位置和车速高频5s上报
			...
			//上报车况信息和故障事件
				...
				dataLen += DATA_TIMESTAMP_SIZE + DATA_AUTHTOKEN_SIZE;
				//uint16_t FauktLen = FaultSignDeal(ElementBuff + dataLen);
				//dataLen += FauktLen;
			//上报车况信息(全部信号数据)
			}else{
				FAWACPLOG("***************************** Report Vehicle data*********************");
				dataLen = ReportVehicleCondDeal(ElementBuff + DATA_TIMESTAMP_SIZE + DATA_AUTHTOKEN_SIZE, 1);
				dataLen += DATA_TIMESTAMP_SIZE + DATA_AUTHTOKEN_SIZE;
			}
			break;
	...
	FAWACPLOG("####################### FAWACP SKey:");
	for(uint8_t i = 0; i < sizeof(p_FAWACPInfo_Handle->SKey); i++)
		printf("%02x ", p_FAWACPInfo_Handle->SKey[i]);
	printf("\n\n");

	Encrypt_len = Encrypt_AES128Data(p_FAWACPInfo_Handle->SKey, ElementBuff, dataLen, EncryptBuff, CryptFlag);
	FAWACPLOG("Encrypt PayloadData length==%d\n",Encrypt_len);
...
// Packet EOF
	...
	FAWACPLOG("packdata length==%d\n",(uint16_t)(pos-dataBuff));
	for(uint16_t i = 0; i < (uint16_t)(pos-dataBuff); i++)
		FAWACP_NO("%02x", *(dataBuff + i));
	FAWACP_NO("\n\n\n");
...
uint16_t CFAWACP::Unpack_FAWACP_FrameData(uint8_t *pData, int32_t DataLen)
{
	...
	FAWACPLOG("Decrypt_AES128Data SKey_TBox:");
	for(uint8_t i = 0; i < sizeof(SKey_TBox); i++)
		printf("%02x ", SKey_TBox[i]);
	printf("\n\n");
...
uint16_t CFAWACP::AuthApplyDataDeal(uint8_t *pTemp)
{
	...
	*pos++ = AcpTBox_AuthApplyMsg.TU_VIN.Element_LenInfo.Element_Len;
	...
	FAWACPLOG("AuthApplyDataDeal:");	
	uint8_t i = 0;
	for(i = 0; i < sizeof(AcpTBox_AuthApplyMsg.TU_VIN.CarTU_ID); i++)
		printf("%02x ", AcpTBox_AuthApplyMsg.TU_VIN.CarTU_ID[i]);
	printf("AcpTBox_AuthApplyMsg.TU_VIN.CarTU_ID _length = %d\n\n ",i);
...
uint16_t CFAWACP::addRemainedOil(uint8_t* pTemp)
{
	...
	*pos++ = p_FAWACPInfo_Handle->VehicleCondData.RemainedOil;
...
uint16_t CFAWACP::addPastRecordSpeed(uint8_t* pTemp)
{
	...
	for(int i = 0; i < 19; i++)
	{
		...
		{
			Decrypt_AES128Data(p_FAWACPInfo_Handle->SKey, (dataBuff + SIGNATURE_SIZE + DATA_SOFEOF_SIZE), (dataLen - SIGNATURE_SIZE - DATA_SOFEOF_SIZE*2), PayloadBuff, AcpCryptFlag_IS);
...
uint16_t CFAWACP::ReportVehicleCondDeal(uint8_t* dataBuff, uint8_t flag)
{
	...
	*pos++ = CollectPacketSN++;
	...
	if(flag == 1)
		...
		signCodeTotal = MAX_SIGNCODE_TOTAL - 2 + MAX_DEFAULTSIGN_TOTAL;
					...
					*pos++ = m_AcpRemoteConfig.DeviceConfig[i].ConfigItem[j].ConfigItemNum;	//配置项编号
					switch(m_AcpRemoteConfig.DeviceConfig[i].ConfigItem[j].ConfigItemNum)
					{
						...
						case ACPCfgItem_ChargeScheduleID:			//预约充电时间表
							*pos++ = p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[k].ChargeSchedule.DayAppointment;			//预约充电日1-7—>周一到周日
							*pos++ = (p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[k].ChargeSchedule.StartTime >> 8) & 0xFF;	//充电开始时间,单位分钟
							*pos++ = p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[k].ChargeSchedule.StartTime & 0xFF;
							*pos++ = (p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[k].ChargeSchedule.KeepTime >> 8) & 0xFF;	//持续时间,单位分钟
							*pos++ = p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[k].ChargeSchedule.KeepTime & 0xFF;
							break;
						default:
							break;
					}
...
int CFAWACP::ShowdataTime(uint8_t *pos)
{
	...
	sprintf((char *)Tbuff, "%d-%d-%d-%d-%d-%d",year, month, day, hour, minutes, seconds);
	FAWACPLOG("Time Stamp: %s\n", Tbuff);
	
	if(m_loginState == 1){
		m_tspTimestamp.Year = year;
	...
	switch(MsgType)
	{
		...
		case 8:		//Auth Ready ACK Message
			...
			for(uint8_t i = 0; i < 4; i++)
				printf("%02x ", p_FAWACPInfo_Handle->AuthToken[i]);
			printf("\n");
...
uint16_t CFAWACP::UnpackData_AcpRemoteCtrl(uint8_t *PayloadBuff, uint16_t payload_size, uint8_t MsgType)
{
	...
	uint8_t	SubitemCodeParam = 0;//子项编码参数值
	//时间戳部分
	ShowdataTime(pos);
	...
	switch(MsgType)
	{
		case 1:
			...
			for(uint16_t i = 0; i < m_AcpRemoteCtrlList.SubitemTotal; i++)
			{
				...
				{
					case VehicleBody_LockID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleBody.VehicleBody_Lock = *pos++;
						FAWACPLOG("VehicleBody_Lock == %d",SubitemCodeParam);
						break;
					case VehicleBody_WindowID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleBody.VehicleBody_Window = *pos++;
						FAWACPLOG("VehicleBody_Window == %d",SubitemCodeParam);
						break;
					case VehicleBody_SunroofID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleBody.VehicleBody_Sunroof = *pos++;
						FAWACPLOG("VehicleBody_Sunroof == %d",SubitemCodeParam);
						break;
					case VehicleBody_TrackingCarID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleBody.VehicleBody_TrackingCar = *pos++;
						FAWACPLOG("VehicleBody_TrackingCar == %d",SubitemCodeParam);
						break;
					case VehicleBody_LowbeamID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleBody.VehicleBody_Lowbeam = *pos++;
						FAWACPLOG("VehicleBody_Lowbeam == %d",SubitemCodeParam);
						break;
					case VehicleBody_LuggageCarID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleBody.VehicleBody_LuggageCar = *pos++;
						FAWACPLOG("VehicleBody_LuggageCar == %d",SubitemCodeParam);
						break;					
					case Airconditioner_ControlID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_Control = *pos++;
						FAWACPLOG("Airconditioner_Control == %d",SubitemCodeParam);
						break;
					case Airconditioner_CompressorSwitchID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_CompressorSwitch = *pos++;
						FAWACPLOG("Airconditioner_CompressorSwitch == %d",SubitemCodeParam);
						break;
					case Airconditioner_TemperatureID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_Temperature.BitType = *pos++;
						FAWACPLOG("Airconditioner_Temperature == %d",SubitemCodeParam);
						break;
					case Airconditioner_SetAirVolumeID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_SetAirVolume.BitType = *pos++;
						FAWACPLOG("Airconditioner_SetAirVolume == %d",SubitemCodeParam);
						break;
					case Airconditioner_FrontDefrostSwitchID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_FrontDefrostSwitch = *pos++;
						FAWACPLOG("Airconditioner_FrontDefrostSwitch == %d",SubitemCodeParam);
						break;
					case Airconditioner_HeatedrearID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_Heatedrear = *pos++;
						FAWACPLOG("Airconditioner_Heatedrear == %d",SubitemCodeParam);
						break;
					case Airconditioner_BlowingModeID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_BlowingMode.BitType = *pos++;
						FAWACPLOG("Airconditioner_BlowingMode == %d",SubitemCodeParam);
						break;
					case Airconditioner_InOutCirculateID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_InOutCirculate.BitType = *pos++;
						FAWACPLOG("Airconditioner_InOutCirculate == %d",SubitemCodeParam);
						break;
					case Airconditioner_AutoSwitchID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.Airconditioner.Airconditioner_AutoSwitch = *pos++;
						FAWACPLOG("Airconditioner_AutoSwitch == %d",SubitemCodeParam);
						break;
					case EngineState_SwitchID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.EngineState.EngineState_Switch = *pos++;
						FAWACPLOG("EngineState_Switch == %d",SubitemCodeParam);
						break;
					case VehicleSeat_DrivingSeatID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleSeat.VehicleSeat_DrivingSeat = *pos++;
						FAWACPLOG("VehicleSeat_DrivingSeat == %d",SubitemCodeParam);
						break;
					case VehicleSeat_CopilotseatID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleSeat.VehicleSeat_Copilotseat = *pos++;	
						FAWACPLOG("VehicleSeat_Copilotseat == %d",SubitemCodeParam);
						break;
					case VehicleSeat_DrivingSeatMomeryID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleSeat.VehicleSeat_DrivingSeatMemory = *pos++;
						FAWACPLOG("VehicleSeat_DrivingSeatMemory == %d",SubitemCodeParam);
						break;
					case VehicleChargeMode_ImmediateID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleCharge.VehicleCharge_Immediate = *pos++;
						FAWACPLOG("VehicleCharge_Immediate == %d",SubitemCodeParam);
						break;
					case VehicleChargeMode_AppointmentID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleCharge.VehicleCharge_Appointment = *pos++;
						FAWACPLOG("VehicleCharge_Appointment == %d",SubitemCodeParam);
						break;
					case VehicleWIFIStatusID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleAutopark.VehicleWifiStatus = *pos++;
						FAWACPLOG("VehicleWifiStatus == %d",SubitemCodeParam);
						break;
					case VehicleAutoOUTID:
						SubitemCodeParam = m_AcpRemoteCtrlCommandData.VehicleAutopark.VehicleAutoOut = *pos++;
						FAWACPLOG("VehicleAutoOut == %d",SubitemCodeParam);
						break;
					default:
						break;
				}
				reportRemoteCtrlCmd(mcuUart::m_mcuUart->cb_RemoteCtrlCmd,m_AcpRemoteCtrlList.SubitemCode[i], SubitemCodeParam, m_AcpRemoteCtrlList.SubitemTotal);
			}
			break;
		case 0:
			if(search(Headerlink, ACPApp_RemoteCtrlID) != NULL)
...
//回调函数控制MCU发送远程控制指定
void CFAWACP::reportRemoteCtrlCmd(callback_RemoteCtrlCmd cb_RemoteCtrlCmd, uint8_t SubitemCode, uint8_t SubitemCodeParam, uint8_t SubitemTotal)
{
	FAWACPLOG("CFAWACP: reportRemoteCtrlCmd");

	cb_RemoteCtrlCmd(SubitemCode, SubitemCodeParam, SubitemTotal);
}
...
uint16_t CFAWACP::UnpackData_AcpQueryVehicleCond(uint8_t *PayloadBuff, uint16_t payload_size, uint8_t applicationID, uint8_t MsgType)
{
	...
	switch(MsgType)
	{
		case 1:
			...
			pos += 2;
			FAWACPLOG("m_AcpVehicleCondCommandList.CollectTime == %d\n", m_AcpVehicleCondCommandList.CollectTime);
			FAWACPLOG("m_AcpVehicleCondCommandList.CollectCount == %d\n", m_AcpVehicleCondCommandList.CollectCount);
			FAWACPLOG("m_AcpVehicleCondCommandList.SignTotal == %d\n", m_AcpVehicleCondCommandList.SignTotal);
		
			//for(uint8_t i = 0; i < m_AcpVehicleCondCommandList.SignTotal; i++)
			//	m_AcpVehicleCondCommandList.SignCode[i] = *pos++;
				...
				for(uint8_t k = 0; k < m_AcpRemoteConfig.DeviceConfig[i].ConfigCount; k++)
				{
					...
					switch(m_AcpRemoteConfig.DeviceConfig[i].ConfigItem[k].ConfigItemNum)
					{
						...
						case ACPCfgItem_ChargeScheduleID:
							//预约充电时间表
							p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[i].ChargeSchedule.DayAppointment = *pos++;//预约充电日
							p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[i].ChargeSchedule.StartTime = ((pos[0] << 8) & 0xFF00) | (pos[1] & 0x00FF);//预约充电开始时间
							pos += 2;
							p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[i].ChargeSchedule.KeepTime = ((pos[0] << 8) & 0xFF00) | (pos[1] & 0x00FF);//充电持续时间
							pos += 2;
							FAWACPLOG("DayAppointment == %d\n",p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[i].ChargeSchedule.DayAppointment);
							FAWACPLOG("StartTime == %d\n",p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[i].ChargeSchedule.StartTime);
							FAWACPLOG("KeepTime == %d\n",p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[i].ChargeSchedule.KeepTime);
							break;
						default:
							break;
					}
...
uint16_t CFAWACP::UnpackData_AcpUpdateRootKey(uint8_t *PayloadBuff, uint16_t payload_size, uint8_t MsgType)
{
	...
	switch(MsgType)
	{
		case 2:
			...
			Decrypt_AES128Data(p_FAWACPInfo_Handle->RootKey, pos, 32, p_FAWACPInfo_Handle->New_RootKey, AcpCryptFlag_IS);
			pos += 32;
		...
		case 0:
			...
			memcpy(p_FAWACPInfo_Handle->RootKey,p_FAWACPInfo_Handle->New_RootKey,ROOTKEY_SIZE);
