...
void *mcuUart::CheckPWMThread(void *arg)
{
	...
	int InitFlagCheckPWM = 0; //初始化PWM200MS
	int nVoltage_SRS 	 = 0;//检测SRS电压值
	int nPeriodCheckPWM  = 0;//检测高电平的周期
	struct timeval tvBeginCheckPWM, tvPeriodCheckPWM;//数据上报时间间隔ms
 	gettimeofday(&tvBeginCheckPWM, NULL);
	gettimeofday(&tvPeriodCheckPWM, NULL);

	while(1)
	{
		gettimeofday(&tvPeriodCheckPWM, NULL);
		long cost_tvtime = 0;
		cost_tvtime = (tvPeriodCheckPWM.tv_sec*1000 + tvPeriodCheckPWM.tv_usec/1000) - (tvBeginCheckPWM.tv_sec*1000 + tvBeginCheckPWM.tv_usec/1000);
		if(cost_tvtime == 40)//40ms计量一次
		{
			printf(" ############# Deployment Cammand SRS begin ############# \n");
			//检测SRS电压
			if(adc_voltage_check(&nVoltage_SRS) == 0)
			{
				printf("nInitCheckPWM Current nVoltage_SRS:%d mv\n", nVoltage_SRS);
		...
		if(InitFlagCheckPWM == 0)//初始化状态
		{
			if( (nNumCheckHighVol == 5 && nNumCheckLowVol == 0) || 
				(nNumCheckHighVol == 0 && nNumCheckLowVol == 5) )			
			{
				nNumCheckHighVol = 0;
				nNumCheckLowVol  = 0;
				InitFlagCheckPWM = 1;
		...
		else if(InitFlagCheckPWM == 1)//开始检测
		{
			if(nNumCheckHighVol == 5 && nNumCheckLowVol == 1)//正常输出
			{
				nNumCheckHighVol = 0;
				nNumCheckLowVol  = 0;
				nPeriodCheckPWM  = 0;
			...
			if(nNumCheckHighVol == 1 && nNumCheckLowVol == 5)//碰撞周期
			{
				nPeriodCheckPWM++;
...
void *mcuUart::CheckEventThread(void *arg)
{
	...
	int iRet = -1;
	int iLoop = 0;
	static int iflagDoorIntrusAlarm = 0;
	static int iflagRemainUpkeepMileage = 0;
	static int iflagVehOutFileAlarm = 0; //紧急熄火报警
	...
	while(1)
	{
		...
		if(CFAWACP::cfawacp->m_loginState == 2 && CFAWACP::cfawacp->m_ConnectedState == true)
		{
			...
			if(pmcuUart->SigEvent.s_DoorIntrusAlarm == 1 && iflagDoorIntrusAlarm == 0)	//车门入侵
			{
				pmcuUart->reportEventCmd(CFAWACP::cfawacp->timingReportingData, 1, ACPApp_VehicleAlarmID);
				iflagDoorIntrusAlarm = 1;
			}

			//保养提醒报警	暂时定剩余保养里程小于500时为保养提醒
			if(p_FAWACPInfo_Handle->VehicleCondData.RemainUpkeepMileage <= 500 && iflagRemainUpkeepMileage == 0)
			{
				pmcuUart->reportEventCmd(CFAWACP::cfawacp->timingReportingData, 3, ACPApp_VehicleAlarmID);
				iflagRemainUpkeepMileage = 1;
			}
			...
			if(pmcuUart->SigEvent.s_VehOutFileAlarm == 1 && iflagVehOutFileAlarm == 0)			//紧急熄火报警
			{
				pmcuUart->reportEventCmd(CFAWACP::cfawacp->timingReportingData, 5, ACPApp_VehicleAlarmID);
				iflagVehOutFileAlarm = 1;
			}
...
int mcuUart::unpack_RemoteCtrl(unsigned char *pData, uint16_t datalen)
{
	...
	MCULOG("====== Begin unpack_RemoteCtrl. ======\n");
...
{
	MCULOG("==== Begin MCU Send RemoteCtrl ====");
	printf("SubitemTotal == %d\n",SubitemTotal);
	int i,headLen;
	int totalDataLen;
	unsigned char *pos = NULL;
...
//	pos += dataLen;
	...
	*pos++ = 0x7e;
	...
	MCULOG("====  MCU Send RemoteCtrl End ====");
	return 0;
}
