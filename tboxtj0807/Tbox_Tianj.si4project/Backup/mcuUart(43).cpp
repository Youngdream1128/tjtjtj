	...
	while(1)
	{
		...
		else
		{
			if(pmcuUart->SigEvent.s_VehCollideAlarm == 1)
			{
				...
				tmp_time = time(NULL);
				p_tm = gmtime(&tmp_time);
				printf("@@@@@@@@@@@@@ connect &login is fail: CheckEventThread voiceCall 13424402096 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
				printf(" %0d-%d-%d-%d-%d-%d\n",p_tm->tm_year+1900, p_tm->tm_mon, p_tm->tm_mday,p_tm->tm_hour,p_tm->tm_min,p_tm->tm_sec);
...
//			MCULOG("====== GYL unpack_updateTimingInfo DATA end ======\n");
		...
		case TEXT_TO_SPEECH_ID:
			...
			break;
		case MCU_QUERY_SLEEP_MODE:
			unpack_querySleepMode(pData, datalen);
			break;
		case MCU_GENERAL_RESP:
		//	unpack_querySleepMode(pData, datalen);
			break;
...
int mcuUart::unpack_updatePositionInfo(unsigned char *pData, unsigned int len)
{
	...
	//printf("####################  unpack_updateFAWACPVehCondInfo start voiceCall_____1 #################### :%0d-%d-%d-%d-%d-%d\n",p_tm->tm_year+1900, p_tm->tm_mon, p_tm->tm_mday,p_tm->tm_hour,p_tm->tm_min,p_tm->tm_sec);
	unpack_updateFAWACPVehCondInfo(pData, len);
	//printf("####################  unpack_updateFAWACPVehCondInfo start voiceCall_____2 #################### :%0d-%d-%d-%d-%d-%d\n",p_tm->tm_year+1900, p_tm->tm_mon, p_tm->tm_mday,p_tm->tm_hour,p_tm->tm_min,p_tm->tm_sec);
...
//	MCULOG("frontfoglights == %d\n",p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.frontfoglights );
	...
	//p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.overspeedTime = (pData[81]<<8) + pData[82];
	p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.overspeedTime = (pData[81]<<8) + 0x00;
	...
	{	
		...
		tmp_time = time(NULL);
		p_tm = gmtime(&tmp_time);
		printf("@@@@@@@@@ unpack_updateFAWACPVehCondInfo s_VehCollideAlarm 13424402096 @@@@@@@@@@@@@\n");
		printf(" %0d-%d-%d-%d-%d-%d\n",p_tm->tm_year+1900, p_tm->tm_mon, p_tm->tm_mday,p_tm->tm_hour,p_tm->tm_min,p_tm->tm_sec);
	}
	else
	{
		printf("@@@@@@@@@ unpack_updateFAWACPVehCondInfo s_VehCollideAlarm == 0 @@@@@@@@@@@@@\n");
	}
