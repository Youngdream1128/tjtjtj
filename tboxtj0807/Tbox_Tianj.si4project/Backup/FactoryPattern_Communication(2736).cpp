...
uint8_t FactoryPattern_Communication::pack_TBOX_Data_Report(uint8_t TestFlag, uint16_t MsgID, uint8_t callState)
{
	int i;
	uint32_t seconds;
	uint16_t dataLen;
...
uint16_t FactoryPattern_Communication::pack_Protocol_Data(uint8_t *pData, int len, DISPATCHER_MSG_ST *dispatchMsg, uint8_t state)
{
	int i, ret;
	uint16_t dataLen;
	uint16_t totalLen;
...
uint8_t FactoryPattern_Communication::TBOX_Voicall_State(uint8_t *pData)
{
	int i;
...
uint32_t FactoryPattern_Communication::Get_Seconds_from_1970()
{
	uint32_t seconds;
    time_t tv;
   //struct tm *p;
