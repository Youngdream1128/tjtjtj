...
int mcuUart::unpack_updateFAWACPVehCondInfo(unsigned char* pData, unsigned int len)
{
	...
	//长春一汽
	p_FAWACPInfo_Handle->VehicleCondData.CurrentSpeed = (pData[37] << 8) + pData[38];	//当前车速
	p_FAWACPInfo_Handle->VehicleCondData.LTAverageSpeed = ((pData[39] << 8) & 0xFF00) | (pData[40] & 0xFF);//长时平均车速
	p_FAWACPInfo_Handle->VehicleCondData.STAverageSpeed = ((pData[41] << 8) & 0xFF00) | (pData[42] & 0xFF);//短时平均车速

	p_FAWACPInfo_Handle->VehicleCondData.LTAverageOil = ((pData[43] << 8) & 0xFF00) | (pData[44] & 0xFF);//长时平均油耗
	p_FAWACPInfo_Handle->VehicleCondData.STAverageOil = ((pData[45] << 8) & 0xFF00) | (pData[46] & 0xFF);//短时平均油耗
...
//	MCULOG("STAverageOil == %d\n",p_FAWACPInfo_Handle->VehicleCondData.STAverageOil);
	//车门状态
	p_FAWACPInfo_Handle->VehicleCondData.CarDoorState.bitState.drivingDoor = pData[47] & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarDoorState.bitState.copilotDoor = (pData[47] >> 1) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarDoorState.bitState.leftRearDoor = (pData[47] >> 2) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarDoorState.bitState.rightRearDoor = (pData[47] >> 3) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarDoorState.bitState.rearCanopy = (pData[47] >> 4) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarDoorState.bitState.engineCover = (pData[47] >> 5) & 0x01;
...
//	MCULOG("engineCover == %d\n",p_FAWACPInfo_Handle->VehicleCondData.CarDoorState.bitState.engineCover);
	//车锁状态
	p_FAWACPInfo_Handle->VehicleCondData.CarLockState.bitState.rightRearLock = pData[48] & 0x03;
	p_FAWACPInfo_Handle->VehicleCondData.CarLockState.bitState.leftRearLock = (pData[48] >> 2) & 0x03;
	p_FAWACPInfo_Handle->VehicleCondData.CarLockState.bitState.copilotLock = (pData[48] >> 4) & 0x03;
	p_FAWACPInfo_Handle->VehicleCondData.CarLockState.bitState.drivingLock = (pData[48] >> 6) & 0x03;
...
//	MCULOG("drivingLock == %d\n",p_FAWACPInfo_Handle->VehicleCondData.CarLockState.bitState.drivingLock);
	//天窗状态
	p_FAWACPInfo_Handle->VehicleCondData.sunroofState = pData[49];
	
//	MCULOG("sunroofState == %d\n",p_FAWACPInfo_Handle->VehicleCondData.sunroofState);
	//车窗状态数据
	p_FAWACPInfo_Handle->VehicleCondData.WindowState.bitState.leftFrontWindow = pData[51] & 0x07;
	p_FAWACPInfo_Handle->VehicleCondData.WindowState.bitState.rightFrontWindow = (pData[51] >> 3) & 0x07;
	p_FAWACPInfo_Handle->VehicleCondData.WindowState.bitState.leftRearWindow = ((pData[51] >> 6) & 0x07) + ((pData[50] << 2) & 0x04);
	p_FAWACPInfo_Handle->VehicleCondData.WindowState.bitState.rightRearWindow = ((pData[50] >> 1) & 0x07);
...
//	MCULOG("rightRearWindow == %d\n",p_FAWACPInfo_Handle->VehicleCondData.WindowState.bitState.rightRearWindow );
	//车灯状态数据
	p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.headlights = pData[52] & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.positionlights = (pData[52] >> 1) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.nearlights = (pData[52] >> 2) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.rearfoglights = (pData[52] >> 3) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.frontfoglights = (pData[52] >> 4) & 0x01;
...
//	MCULOG("frontfoglights == %d\n",p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.frontfoglights );
	//轮胎信息数据
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.rightrearTyrePress = pData[60];
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.leftrearTyrePress = pData[59];
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.rightfrontTyrePress = pData[58];
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.leftfrontTyrePress = pData[57];
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.rightrearTemperature = pData[56];
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.leftrearTemperature = pData[55];
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.rightfrontTemperature = pData[54];
	p_FAWACPInfo_Handle->VehicleCondData.TyreState.leftfrontTemperature = pData[53];
	//TBox_MCU版本
	memcpy(p_FAWACPInfo_Handle->VehicleCondData.VerTboxMCU, &pData[61], 12);
	//发动机状态
	p_FAWACPInfo_Handle->VehicleCondData.EngineState = pData[73] & 0x07;
	//实时方向盘转角数据
	p_FAWACPInfo_Handle->VehicleCondData.WheelState.bitState.wheeldegree = ((pData[74] & 0x7F) << 8) + pData[75];
	p_FAWACPInfo_Handle->VehicleCondData.WheelState.bitState.wheeldirection = (pData[74] >> 7) & 0x01;
	//发动机转速
	p_FAWACPInfo_Handle->VehicleCondData.EngineSpeed = (pData[76] << 8) + pData[77];
	//档位信息
	p_FAWACPInfo_Handle->VehicleCondData.Gearstate = pData[78] & 0x0F;
	//手刹状态
	p_FAWACPInfo_Handle->VehicleCondData.HandbrakeState = pData[79] & 0x03;
	//电子驻车状态
	p_FAWACPInfo_Handle->VehicleCondData.ParkingState = pData[80] & 0x03;
	//供电模式
	p_FAWACPInfo_Handle->VehicleCondData.Powersupplymode = pData[81] & 0x03;	
	//安全带状态
	p_FAWACPInfo_Handle->VehicleCondData.Safetybeltstate = pData[82] & 0x03;
	//剩余保养里程
	p_FAWACPInfo_Handle->VehicleCondData.RemainUpkeepMileage = (pData[83] << 8) + pData[84];
	//空调相关信息
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.airconditionerState = pData[87] & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.compressorState = (pData[87] >> 1) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.autoState = (pData[87] >> 2) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.defrostState = (pData[87] >> 3) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.inOutCirculateState = (pData[87] >> 4) & 0x01;
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.blowingLevel = (pData[87] >> 5) & 0x07;
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.blowingMode = pData[86] & 0x07;
	p_FAWACPInfo_Handle->VehicleCondData.AirconditionerInfo.Temperature = ((pData[86] >> 3) & 0x1F) + ((pData[85] & 0x03) << 5);
	//持续时间信息
	p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.accelerateTime = pData[91];//急加速持续时间
	p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.decelerateTime = pData[90];//急减速持续时间
	p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.wheelTime = pData[89];//急转弯持续时间
	p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.overspeedTime = (pData[88]<<8) + 0x00;//超速持续时间
	
	//动力电池
	/*预留*/

	//充电状态数据
	p_FAWACPInfo_Handle->VehicleCondData.ChargeState.chargeState = pData[101];
	p_FAWACPInfo_Handle->VehicleCondData.ChargeState.remainChargeTime = pData[100];
	
	//TBOX-OS版本
	/*预留*/
	//memcpy(p_FAWACPInfo_Handle->VehicleCondData.VerTboxOS, "tbox1.0.02", 12);
	//IVI版本
	/*预留*/
	memcpy(p_FAWACPInfo_Handle->VehicleCondData.VerIVI, &pData[114], 16);
	//充电枪连接状态
	p_FAWACPInfo_Handle->VehicleCondData.ChargeConnectState = pData[130];
	//制动踏板开关
	p_FAWACPInfo_Handle->VehicleCondData.BrakePedalSwitch = pData[131];
	//加速踏板开关
	p_FAWACPInfo_Handle->VehicleCondData.AcceleraPedalSwitch = pData[132];
	//Yaw传感器信息
	YaWSensorInfoSwitch_t	YaWSensorInfoSwitch;
	//环境温度
	AmbientTemperat_t	AmbientTemperat;
	//纯电动继电器及线圈相关状态
	PureElecRelayState_t	PureElecRelayState;	
	//剩余续航里程(12bit)
	uint16_t		ResidualRange;
	//新能源热管理相关请求
	NewEnergyHeatManage_t	NewEnergyHeatManage;
	//车辆工作模式
	VehWorkMode_t		VehWorkMode;
	//电机工作状态
	MotorWorkState_t	MotorWorkState;
	//高压系统准备状态
	p_FAWACPInfo_Handle->VehicleCondData.HighVoltageState = pData[147];

	//故障信息
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEMSState = pData[148];		//发动机管理系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTTCUState = pData[149];		//变速箱控制单元故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEmissionState = pData[150];	//排放系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTSRSState = pData[151];		//安全气囊系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTESPState = pData[152];		//电子稳定系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTABSState = pData[153];		//防抱死刹车系统
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEPASState = pData[154];		//电子助力转向系统
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTOilPressureState = pData[155];	//机油压力报警
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTLowOilIDState = pData[156];	//油量低报警
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTBrakeFluidLevelState = pData[157];	//制动液位报警
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTBatteryChargeState = pData[158];	//蓄电池充电故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTBBWState = pData[159];		//制动系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTTPMSState = pData[160];		//胎压系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTSTTState = pData[161];		//启停系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTExtLightState	= pData[162];	//外部灯光故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTESCLState = pData[163];		//电子转向柱锁故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEngineOverwaterState = pData[164];//发动机水温过高报警
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTElecParkUnitState = pData[165];	//电子驻车单元系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTAHBState	= pData[166];	//智能远光系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTACCState = pData[167];		//自适应巡航系统故障

	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTFCWSState	= pData[168];	//前碰撞预警系统故障
  	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTLDWState	= pData[169];	//道路偏离预警系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTBlindSpotDetectState= pData[170];	//盲区检测系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTAirconManualState	= pData[171];	//空调人为操作
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTHVSystemState	= pData[172];	//高压系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTHVInsulateState	= pData[173];	//高压绝缘故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTHVILState	= pData[174];	//高压互锁故障
	// uint8_t   ACPCODEFAULTBatteryChargeState:2;
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEVCellState	= pData[175];	//动力电池故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTPowerMotorState= pData[176];	//动力电机故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEParkState	= pData[177];	//E-Park故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEVCellLowBatteryState	= pData[178];	//动力电池电量过低报警
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTEVCellOverTemperateState= pData[179];	//动力电池温度过高报警
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTPowerMotorOverTemperateState= pData[180];	//动力电机温度过高报警


	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTConstantSpeedSystemFailState= pData[181];	//定速巡航系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTChargerFaultState	    = pData[182];	//充电机故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTAirFailureState		    = pData[183]; //空调故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTAlternateAuxSystemFailState = pData[184]; //换道辅助系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTAutoEmergeSystemFailState   = pData[185];	//自动紧急制动系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTReverRadarSystemFailState   = pData[186];	//倒车雷达系统故障
	p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTElecGearSystemFailState	  = pData[187];	//电子换档器系统故障
	
	//硬线或总线碰撞报警	
	SigEvent.s_VehCollideAlarm = pData[151];
