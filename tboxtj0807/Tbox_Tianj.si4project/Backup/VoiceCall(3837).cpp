...
//int hangUp_answer(unsigned char isAnswer);
...
)
{
    ...
    if(QMI_VOICE_ALL_CALL_STATUS_IND_V02 == msg_id)
    {
        ...
        {   
            /*
            LOG("index[%d] id=%d, state=%d, dir=%d",i,call_info_msg.call_info[i].call_id,
...
uint8_t voice_dial(char *callingnumber)
{
	...
	printf("@@@@@@@@@ simcom voice_dial 13424402096 @@@@@@@@@@@@@\n");
...
* Date          : 2018.03.10
*****************************************************************************/
int voiceCall(char *callingNumber)
{
	struct tm *p_tm = NULL;
	time_t tmp_time;
	tmp_time = time(NULL);
	...
	LOG("^^^^^^^^^^^^^^^^start voiceCall ^^^^^^^^^^^^^^^^^:%0d-%d-%d-%d-%d-%d",p_tm->tm_year+1900, p_tm->tm_mon, p_tm->tm_mday,p_tm->tm_hour,p_tm->tm_min,p_tm->tm_sec);
	
	if(callingNumber == NULL)
		return -1;

	if(strcmp(callingNumber,"110") == 0)
	{
		tboxInfo.operateionStatus.phoneType = 1;
	}
	else if (strcmp(callingNumber,"120") == 0)
	{
		tboxInfo.operateionStatus.phoneType = 2;
	}
	else if((strcmp(callingNumber,"119") == 0))
	{
