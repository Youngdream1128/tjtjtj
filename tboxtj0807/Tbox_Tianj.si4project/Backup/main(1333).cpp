...
//加密
    ...
    //char pRootKey[128] = "12345678123456781234567812345678";//"\x54\xF0\x49\x13\xE5\x20\x8E\xE0\xB8\xEA\xCC\x07\x4D\x91\x24\xFF";
	uint8_t pRootKey[16] = {0x7b, 0xde, 0x18, 0x70, 0x1a, 0xc2, 0x52, 0xb9, 0xa9, 0xb6, 0x8a, 0x08, 0x8c, 0x9a, 0x8f, 0x82};
	uint8_t *encryptedKey;
	encryptedKey = (uint8_t *)malloc(256);
	if(encryptedKey == NULL)
		printf("malloc encryptedKey error!");
	else
		printf("malloc encryptedKey success!\n");
	printf("pRootKey is %s\n",pRootKey);	
	result = encrypt_key(pRootKey, input_len, encryptedKey);
	printf("ntsEncryptKey result:%d.\n",result);
	
//解密
	if(result > 0){
		int lenEncryptedKey = result;
		printf(">>>>>>>>>>>>>>>>>>>Decrypted_key start:\n");
		//uint8_t encryptedKey[32] = {0x7b, 0xde, 0x18, 0x70, 0x1a, 0xc2, 0x52, 0xb9, 0xa9, 0xb6, 0x8a, 0x08, 0x8c, 0x9a, 0x8f, 0x82,0x70, 0x1a, 0xc2, 0x52, 0xb9, 0xa9};
		uint8_t *rootKey;
		rootKey = (uint8_t *)malloc(256);
		if(rootKey == NULL)
			printf("malloc rootKey error!");
		else
			printf("malloc rootKey success!\n");
		result = decrypt_key(encryptedKey, lenEncryptedKey, rootKey);
