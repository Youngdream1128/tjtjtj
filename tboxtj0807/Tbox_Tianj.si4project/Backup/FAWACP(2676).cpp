...
CFAWACP::CFAWACP()
{
	int InLoginFailTimes = 0;
	int OutLoginFailTimes = 0;
	uint8_t flag = 0;
	m_Socketfd = -1;
...
void CFAWACP::Init_FAWACP_dataInfo()
{
	...
	strcat(VerTboxOS, TBOX_VERSION);//FILE_VERSION
...
void CFAWACP::InitAcpTBoxAuthData()
{
	...
	FAWACPLOG("VIN: >>>>>>>>\n ");
	uint8_t i = 0;
	for(i = 0; i < sizeof(p_FAWACPInfo_Handle->Vin); i++)
		printf("%02x ", p_FAWACPInfo_Handle->Vin[i]);
	FAWACPLOG("\n");
	//get ICCID
...
//PayloadBuff 执行了加密后的数据
	switch (applicationID)
	{
		...
		case ACPApp_RemoteCtrlID://远程控制
			...
			printf("============================Cuo WU Ma === %d\n", mcuUart::m_mcuUart->RCtrlErrorCode);
...
// Packet EOF
	...
	printf("==========================================================================================\n");
	uint8_t bufff[512] = {0};
	Decrypt_AES128Data(p_FAWACPInfo_Handle->SKey, dataBuff + 29, (uint16_t )(pos-dataBuff) - 33, bufff, 1);
	
	for(int i = 0; i < 32; i++)
		printf("%02x ",bufff[i+18]);
	printf("\n\n\n");

	uint8_t root[16]= {0};
	Decrypt_AES128Data(p_FAWACPInfo_Handle->New_RootKey, &bufff[18], 32, root, 1);
...
uint16_t CFAWACP::addPastRecordWheelState(uint8_t* pTemp)
{
	...
	for(int i = 0;i < 19; i++)
	{
		...
		{
			ret = Decrypt_AES128Data(p_FAWACPInfo_Handle->SKey, (dataBuff + 29), (dataLen - 33), PayloadBuff, AcpCryptFlag_IS);
			#if 0
	...
	switch(MsgType)
	{
		...
		case 8:		//Auth Ready ACK Message
			...
			if(TimeOutType.UpdateRootkeyFlag == 2)
			{
				...
				//如果新Rootkey未更换成功，这条消息为旧Rootkey加密旧Rootkey，暂时注释
...
uint16_t CFAWACP::UnpackData_AcpUpdateRootKey(uint8_t *PayloadBuff, uint16_t payload_size, uint8_t MsgType)
{
	...
	switch(MsgType)
	{
		case 2:
			...
			RequestSource = *pos++;
			FAWACPLOG("Request Message Source -- %d\n",RequestSource);
			Decrypt_AES128Data(p_FAWACPInfo_Handle->RootKey, pos, 32, New_RootKey, AcpCryptFlag_IS);
