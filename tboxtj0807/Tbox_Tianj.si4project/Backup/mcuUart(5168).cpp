...
{
	fd = -1;
	memset(&SigEventType, 0, sizeof(SIG_EventType_t));
...
//	MCULOG("frontfoglights == %d\n",p_FAWACPInfo_Handle->VehicleCondData.CarlampState.bitState.frontfoglights );
...
#if 1
	printf("EngineState == %d\n", p_FAWACPInfo_Handle->VehicleCondData.EngineState);
	if(EngineState != p_FAWACPInfo_Handle->VehicleCondData.EngineState){
		//发动机状态由OFF变为RUN(Cranking和stoping只是瞬时状态)	
		if(EngineState == 0 && CFAWACP::cfawacp->m_loginState == 2){//车辆启动事件上报
			if(p_FAWACPInfo_Handle->VehicleCondData.EngineState == 2)
				reportEventCmd(CFAWACP::cfawacp->timingReportingData, 5, ACPApp_VehicleCondUploadID);
		}
		//发动机状态由RUN变为OFF
		if(EngineState == 2 && CFAWACP::cfawacp->m_loginState == 2){//车辆熄火事件上报
			if(p_FAWACPInfo_Handle->VehicleCondData.EngineState == 0)
	...
	//Tbox只做透传，不做逻辑处理
		...
		SigEvent.s_VehHealthRemAlarm = 1;
	}else{
		SigEvent.s_VehHealthRemAlarm = 0;
	}
	if(SigEventType.keepMileageType != SigEvent.s_VehHealthRemAlarm){
		if(SigEvent.s_VehHealthRemAlarm != 0 && CFAWACP::cfawacp->m_loginState == 2){
			reportEventCmd(CFAWACP::cfawacp->timingReportingData, 3, ACPApp_VehicleAlarmID);
		}
		SigEventType.keepMileageType = SigEvent.s_VehHealthRemAlarm;
	}
#endif
	//空调相关信息
	...
	//************************故障信息***************************//		
		...
		p_FAWACPInfo_Handle->AcpCodeFault.ACPCODEFAULTDCDCConverterFaultState = pData[198];	 //直流直流转换器故障
	...
	SigEvent.s_DoorIntrusAlarm = pData[235];	//车门入侵报警
	if(SigEvent.s_DoorIntrusAlarm != SigEventType.DoorIntrusType){
		if(SigEvent.s_DoorIntrusAlarm != 0 && CFAWACP::cfawacp->m_loginState == 2){
			reportEventCmd(CFAWACP::cfawacp->timingReportingData, 1, ACPApp_VehicleAlarmID);
		}
		SigEventType.DoorIntrusType = SigEvent.s_DoorIntrusAlarm;
	}

	SigEvent.s_VehOutFileAlarm = pData[236];	//紧急熄火报警
	if(SigEvent.s_VehOutFileAlarm != SigEventType.OutFileType){
		if(SigEvent.s_VehOutFileAlarm != 0 && CFAWACP::cfawacp->m_loginState == 2){
			reportEventCmd(CFAWACP::cfawacp->timingReportingData, 5, ACPApp_VehicleAlarmID);
		}
		SigEventType.OutFileType = SigEvent.s_VehOutFileAlarm;
	}

	SigEvent.s_VehCollideAlarm = pData[237];	//碰撞信号告警
	if(SigEvent.s_VehCollideAlarm != SigEventType.VehCollideType){
		if(SigEvent.s_VehCollideAlarm != 0 && CFAWACP::cfawacp->m_loginState == 2){
			reportEventCmd(CFAWACP::cfawacp->timingReportingData, 1, ACPApp_EmergencyDataID);
		}
		SigEventType.VehCollideType = SigEvent.s_VehCollideAlarm;
	}
	if(SigEvent.s_VehCollideAlarm != SigEventType.VoiceCallType){	
		if(SigEvent.s_VehCollideAlarm != 0){
			for(uint8_t iLoop = 0; iLoop < 3; iLoop++){
			if(voiceCall(p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].EmergedCall) == 0)
				break;
		}
		SigEventType.VoiceCallType = SigEvent.s_VehCollideAlarm;
	}
}

	SigEvent.s_VehMoveAlarm = pData[238];		//车辆异动告警
	if(SigEvent.s_VehMoveAlarm != SigEventType.VehMoveAlarmType){
		if(SigEvent.s_VehMoveAlarm != 0 && CFAWACP::cfawacp->m_loginState == 2){
			reportEventCmd(CFAWACP::cfawacp->timingReportingData, 2, ACPApp_VehicleAlarmID);
		}
		SigEventType.VehMoveAlarmType = SigEvent.s_VehMoveAlarm;
	}

	SigEvent.s_VehSkylightUnclose = pData[239];	//天窗未关提示
	if(SigEvent.s_VehSkylightUnclose != SigEventType.VehSunroofType){
		if(SigEvent.s_VehSkylightUnclose != 0 && CFAWACP::cfawacp->m_loginState == 2){
			printf("===================================Soor sky Not Close=====================\n");
			printf("===================================Soor sky Not Close=====================\n");
			reportEventCmd(CFAWACP::cfawacp->timingReportingData, 7, ACPApp_VehicleAlarmID);
		}
		SigEventType.VehSunroofType = SigEvent.s_VehSkylightUnclose;
	}
}
...
int mcuUart::unpack_RemoteCtrl(unsigned char *pData, uint16_t datalen)
{
	...
	switch(SubitemCode)
	{
		...
		case MCUAir_Control:
			...
			pData += 10;
			for(int i = 0; i< ctrlTotal; i++)
			{
				switch(*pData++)
				{
					case 0x01:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Control.BitType = *pData++;
			break;
					case 0x02:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_CompressorSwitch.BitType = *pData++;
					break;
					case 0x03:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Temperature.BitType = *pData++;
					break;
					case 0x04:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_SetAirVolume.BitType = *pData++;
			break;
					case 0x05:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_FrontDefrostSwitch.BitType = *pData++;
			break;
					case 0x06:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Heatedrear.BitType = *pData++;
			break;
					case 0x07:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_BlowingMode.BitType = *pData++;
			break;
					case 0x08:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_InOutCirculate.BitType = *pData++;
			break;
					case 0x09:
					p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_AutoSwitch.BitType = *pData++;
			break;
					default:
			break;
				}
			}
			printf("Entar AIR Report State!!!\n\n\n");
			break;
	...
	if(MCUVeh_EnterRemoteModeFail != SubitemCode)
		{
                if(ctrlTotal > 1)
		{
                        if(p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Control.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_CompressorSwitch.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Temperature.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_SetAirVolume.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_FrontDefrostSwitch.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_Heatedrear.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_BlowingMode.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_InOutCirculate.BitType == 1 ||
                        p_FAWACPInfo_Handle->RemoteControlData.Airconditioner.Airconditioner_AutoSwitch.BitType == 1)
                                RCtrlErrorCode = 1;
                        else
                                RCtrlErrorCode = 0;
		}
			...
			{
		        if(SubitemCodeParam == MCU_REMOTE_REPORT_SUCCESS)
			        RCtrlErrorCode = 0;
		        if(SubitemCodeParam == MCU_REMOTE_REPORT_FAULT)
				RCtrlErrorCode = 1;
			}
		}
	printf("RCtrlErrorCode == %d\n", RCtrlErrorCode);
		ReplayRemoteCtrl(CFAWACP::cfawacp->cb_TspRemoteCtrl);
        memset(&AcInfo, 0, sizeof(AC_Contrl_Info));
	return 0;
}
	...
	*pos++ = 0;
	...
	switch(SubitemCode)
	{
		...
		case Airconditioner_ControlID:
                        m_mcuUart->AcInfo.AC_Control = SubitemCodeParam;
			m_mcuUart->AcInfo.ControlfType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_CompressorSwitchID:
			m_mcuUart->AcInfo.AC_CompressorSwitch = SubitemCodeParam;
			m_mcuUart->AcInfo.CompressorSwitchType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_TemperatureID:
                        m_mcuUart->AcInfo.AC_Temperature = SubitemCodeParam;
			m_mcuUart->AcInfo.TemperatureType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_SetAirVolumeID:
                        m_mcuUart->AcInfo.AC_SetAirVolume = SubitemCodeParam;
			m_mcuUart->AcInfo.SetAirVolumeType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_FrontDefrostSwitchID:
                        m_mcuUart->AcInfo.AC_FrontDefrostSwitch = SubitemCodeParam;
			m_mcuUart->AcInfo.FrontDefrostSwitchType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_HeatedrearID:
                        m_mcuUart->AcInfo.AC_Heatedrear = SubitemCodeParam;
			m_mcuUart->AcInfo.HeatedrearType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_BlowingModeID:
                        m_mcuUart->AcInfo.AC_BlowingMode = SubitemCodeParam;
			m_mcuUart->AcInfo.BlowingModeType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_InOutCirculateID:
                        m_mcuUart->AcInfo.AC_InOutCirculate = SubitemCodeParam;
			m_mcuUart->AcInfo.InOutCirculateType = 1;
			flag++;
		        goto ACLOOP;
			break;
		case Airconditioner_AutoSwitchID:
                        m_mcuUart->AcInfo.AC_AutoSwitch = SubitemCodeParam;
			m_mcuUart->AcInfo.AutoSwitchType = 1;
			flag++;
			goto ACLOOP;
			break;
...
ACLOOP:
	if(flag == SubitemTotal)
	{
		*pos++ = 0x06;
		*pos++ = SubitemTotal;
		if(1 == m_mcuUart->AcInfo.ControlfType)
		{
			*pos++ = 0x01;
			*pos++ = m_mcuUart->AcInfo.AC_Control;
		}
		if(1 == m_mcuUart->AcInfo.CompressorSwitchType)
		{
			*pos++ = 0x02;
			*pos++ = m_mcuUart->AcInfo.AC_CompressorSwitch;
		}
		if(1 == m_mcuUart->AcInfo.TemperatureType)
		{
			*pos++ = 0x03;
			*pos++ = m_mcuUart->AcInfo.AC_Temperature;
		}
		if(1 == m_mcuUart->AcInfo.SetAirVolumeType)
		{
			*pos++ = 0x04;
			*pos++ = m_mcuUart->AcInfo.AC_SetAirVolume;
		}
		if(1 == m_mcuUart->AcInfo.FrontDefrostSwitchType)
		{
			*pos++ = 0x05;
			*pos++ = m_mcuUart->AcInfo.AC_FrontDefrostSwitch;
		}
		if(1 == m_mcuUart->AcInfo.HeatedrearType)
		{
			*pos++ = 0x06;
			*pos++ = m_mcuUart->AcInfo.AC_Heatedrear;
		}
		if(1 == m_mcuUart->AcInfo.BlowingModeType)
		{
			*pos++ = 0x07;
			*pos++ = m_mcuUart->AcInfo.AC_BlowingMode;
		}
		if(1 == m_mcuUart->AcInfo.InOutCirculateType)
		{
			*pos++ = 0x08;
			*pos++ = m_mcuUart->AcInfo.AC_InOutCirculate;
		}
		if(1 == m_mcuUart->AcInfo.AutoSwitchType)
		{
			*pos++ = 0x09;
			*pos++ = m_mcuUart->AcInfo.AC_AutoSwitch;
		}
goto SENDCTIL;
#if 0
	serialNO++;
	...
	//	pos += dataLen;
		...
		*pos++ = 0x7e;
		...
		for(int i = 0; i < totalDataLen; i++)
			printf("%02x ", *(pBuff+i));
		printf("\n");
		memset(&m_mcuUart->AcInfo, 0, sizeof(AC_Contrl_Info));
		if(m_mcuUart->escape_mcuUart_data(pBuff, totalDataLen) == -1)
		{
			MCULOG();
			return -1;
...
//	pos += dataLen;
	...
	*pos++ = 0x7e;
	...
	totalDataLen = pos-pBuff;
	printf("pei zhi qi dong shi chang SubitemVal == %d\n", SubitemVal);
	printf("totalDataLen =%d", totalDataLen);
