	...
	while(1)
	{
		if(m_ConnectedState == true)
		{
			...
			else if(m_loginState == 2)
			{
				timeHighReport = time(NULL);
				timeNormalReport = time(NULL);
				if(difftime(timeHighReport, timeBegin) == p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeGpsSpeedInterval)
				{  
				...
				if(difftime(timeNormalReport, timeBegin) == p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeInterval)
				{
					//低频上报普通车况 MsgType == 2
