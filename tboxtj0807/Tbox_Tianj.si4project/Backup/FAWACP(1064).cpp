...
*/
	...
	time_t timeHighBegin,timeNormalBegin, timeHighReport,timeNormalReport; //数据上报时间间隔
	timeHighBegin	 = time(NULL); 
	timeNormalBegin	 = time(NULL); 
	timeHighReport	 = time(NULL);
	timeNormalReport = time(NULL);
//保持连接-
	while(1)
	{
		if(m_ConnectedState == true)
		{
			...
			{
				timeHighReport = time(NULL);				
				if(difftime(timeHighReport, timeHighBegin) == p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeGpsSpeedInterval)
				{  
				...
				timeNormalReport = time(NULL);
				if(difftime(timeNormalReport, timeNormalBegin) == p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[0].ReportTimeInterval)
				{
					//低频上报普通车况 MsgType == 2
