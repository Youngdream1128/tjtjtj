...
int message_init()
{
	...
	//send_message(1,"18565753539", (unsigned char *)"hello, this is test message!", strlen("hello, this is test message!"));

	//send_message(2, "18565753539", (unsigned char *)("加油!"), strlen("加油!"));

    /*
	int ucs_len;
	unsigned short ucs_data[100] = {0};
	unsigned char tmp_val_h;
	unsigned char tmp_val_l;
    codeconvert_utf82unicodeStr("你好吗！", ucs_data, &ucs_len);
	for(int i = 0; i < ucs_len; i++)
	{
		tmp_val_l = ucs_data[i] & 0xFF;
		tmp_val_h = (ucs_data[i] & 0xFF00) >> 8;
		ucs_data[i] = (((unsigned short)tmp_val_l << 8) & 0xFF00) + tmp_val_h;
    }
    
	send_message(2, "13564886322", (unsigned char *)ucs_data, ucs_len * 2);
	*/
