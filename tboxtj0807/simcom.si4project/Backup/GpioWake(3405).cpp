...
int lowPowerMode(int isSleep, int isCloseWifi)
{
	...
	if(isSleep == 0)
	{
		...
		uart_wakeup(0);
		//关闭wifi
		DEBUGLOG(" 00 isCloseWifi:%d  openOrCloseWifi:%d tboxInfo.operateionStatus.wifiStartStatus %d\n",isCloseWifi, openOrCloseWifi, tboxInfo.operateionStatus.wifiStartStatus);
		if(((isCloseWifi == 0) && (openOrCloseWifi == true)) || ((isCloseWifi == 0) && (tboxInfo.operateionStatus.wifiStartStatus == 0)))
		{
			DEBUGLOG(" ############### 1111 \n");
			//close wifi success
	...
	{
		FC_WAKE;
		DEBUGLOG(" 11 isCloseWifi:%d  openOrCloseWifi:%d tboxInfo.operateionStatus.wifiStartStatus %d\n",isCloseWifi, openOrCloseWifi, tboxInfo.operateionStatus.wifiStartStatus);
		...
		uart_wakeup(1);
		//打开wifi
		if((isCloseWifi == 1) && (openOrCloseWifi == false))
		{
			DEBUGLOG(" ############### 1111 \n");
			//open wifi success
			if(wifi_OpenOrClose(1) == 0){
