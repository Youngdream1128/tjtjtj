...
void *CFAWACP::HighReportThread(void *arg)
{
	...
	{
		usleep(100);
		if(pCFAWACP->m_ConnectedState == true && pCFAWACP->m_loginState == 1)
		{
...
{
#if DEV_NO == 1
	//服务器提供   memcpy(p_FAWACPInfo_Handle->IMEI, "864881025884101", strlen("864881025884101"));
	memcpy(p_FAWACPInfo_Handle->Vin, "LMGFE3G88D1000602", strlen("LMGFE3G88D1000602"));
	memcpy(p_FAWACPInfo_Handle->ICCID, "89860117750009072266", strlen("89860117750009072266"));
	memcpy(p_FAWACPInfo_Handle->IMEI, "861477034976124", strlen("861477034976124"));
	memcpy(p_FAWACPInfo_Handle->TBoxSerialNumber, "7905030LLH68XSQ170825A00010000", strlen("7905030LLH68XSQ170825A00010000"));
#elif DEV_NO == 2
	//memcpy(p_FAWACPInfo_Handle->IMEI, "8672230276364907", strlen("8672230276364907"));
	memcpy(p_FAWACPInfo_Handle->Vin, "LMGFE3G88D1000603", strlen("LMGFE3G88D1000603"));
	memcpy(p_FAWACPInfo_Handle->ICCID, "89860117750009072274", strlen("89860117750009072274"));
	memcpy(p_FAWACPInfo_Handle->IMEI, "861477034976124", strlen("861477034976124"));
	memcpy(p_FAWACPInfo_Handle->TBoxSerialNumber, "7905030LLH68XSQ170825A00020000", strlen("7905030LLH68XSQ170825A00010000"));
#endif
	memset(p_FAWACPInfo_Handle->IVISerialNumber, 0x30, sizeof(p_FAWACPInfo_Handle->IVISerialNumber));

	uint8_t pRootKey[] = {0x7b, 0xde, 0x18, 0x70, 0x1a, 0xc2, 0x52, 0xb9, 0xa9, 0xb6, 0x8a, 0x08, 0x8c, 0x9a, 0x8f, 0x82};
	memcpy(RootKey, pRootKey, sizeof(RootKey));
...
//	p_FAWACPInfo_Handle->CollectTime.CollectPacketSN = 0;
	...
	p_FAWACPInfo_Handle->VehicleCondData.GPSData.year = p_tm->tm_year;
	...
	p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.overspeedTime = 0xFE;//超速持续时间
	...
	memcpy(p_FAWACPInfo_Handle->VehicleCondData.VerTboxOS,VerTboxOS,12);//TBOX版本号
	memcpy(p_FAWACPInfo_Handle->VehicleCondData.VerIVI, VerIVI, 16);//IVI版本号
	...
	memcpy(p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[1].EmergedCall,EmergedCall,15);//紧急服务号码配置(ASCII码)
	memcpy(p_FAWACPInfo_Handle->RemoteDeviceConfigInfo[1].WhitelistCall,WhitelistCall,15);//白名单号码配置(ASCII码)
...
int	CFAWACP::sendLoginCTMsg()
{
	...
	Decrypt_AES128Data(RootKey, AcpTsp_AkeyMsg.AkeyC_Tsp, sizeof(AcpTsp_AkeyMsg.AkeyC_Tsp), AKey_TBox, AcpCryptFlag_IS);
	FAWACPLOG("AKey_TBox:");
	for(uint8_t i = 0; i < sizeof(AKey_TBox); i++)
		printf("%02x ", AKey_TBox[i]);
	printf("\n\n");
	...
	//生成随机数Rand2_Tbox
	HMACMd5_digest((const char *)RootKey, AcpTBox_CTMsg.Rand1CT_TBox, sizeof(AcpTBox_CTMsg.Rand1CT_TBox), AcpTBox_CTMsg.Rand2_Tbox);
...
int	CFAWACP::sendLoginRSMsg()
{
	...
	HMACMd5_digest((const char *)RootKey, Rand2CT_TBox, 32, AcpTBox_RSMsg.Rand3_TBox);
...
//PayloadBuff 执行了加密后的数据
	...
	Encrypt_len = Encrypt_AES128Data(SKey_TBox, ElementBuff, dataLen, EncryptBuff, CryptFlag);
...
uint16_t CFAWACP::Unpack_FAWACP_FrameData(uint8_t *pData, int32_t DataLen)
{
	...
	Decrypt_len = Decrypt_AES128Data(SKey_TBox, (pos+DATA_HEADER_SIZE), payload_size, PayloadBuff, (AcpCryptFlag_E)Header.CryptFlag);
...
void CFAWACP::TimeStampPart(uint8_t* pTemp, TimeStamp_t *pTimeStamp)
{
	if(NULL != pTemp && NULL != pTimeStamp)
	{
		...
		pTimeStamp->Year = p_tm->tm_year - 90;//这里有函数处理
...
uint16_t CFAWACP::addPastRecordSpeed(uint8_t* pTemp)
{
	...
	for(uint i = 0;i < 19; i++)
	{
		...
		{
			Decrypt_AES128Data(SKey_TBox, (dataBuff + SIGNATURE_SIZE + DATA_SOFEOF_SIZE), (dataLen - SIGNATURE_SIZE - DATA_SOFEOF_SIZE*2), PayloadBuff, AcpCryptFlag_IS);
...
uint16_t CFAWACP::addKeepingstateTime(uint8_t* pTemp)
{
	...
	*pos++ = p_FAWACPInfo_Handle->VehicleCondData.KeepingstateTime.overspeedTime;
...
{
//	AcpVehicleCondition_RawData  AcpVehicleCondition;//待定
	uint16_t tempLen = 0;
...
uint16_t CFAWACP::RootKeyDataDeal(uint8_t *dataBuff)
{
  ...
  Encrypt_AES128Data(p_FAWACPInfo_Handle->New_RootKey, p_FAWACPInfo_Handle->New_RootKey, sizeof(RootKey), pos, AcpCryptFlag_IS);
  pos += 32;
...
uint16_t CFAWACP::UnpackData_AcpUpdateRootKey(uint8_t *PayloadBuff, uint16_t payload_size, uint8_t MsgType)
{
	...
	switch(MsgType)
	{
		case 2:
			...
			Decrypt_AES128Data(RootKey, pos, 32, p_FAWACPInfo_Handle->New_RootKey, AcpCryptFlag_IS);
			pos += 32;
		...
		case 0:
			...
			memcpy(RootKey,p_FAWACPInfo_Handle->New_RootKey,ROOTKEY_SIZE);
