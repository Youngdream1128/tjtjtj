    ...
    for(i = 0; i < number; i++)
    {
        ...
        else if(events[i].events & EPOLLIN)
        {
            ...
            else
            {
				...
				ofstream testrcv;
   				testrcv.open("/data/factorytestrcv.txt", ios::app); 
				for(i = 0; i < dataLen; ++i)
				{
					testrcv << hex << *(buff + i) << endl;
				...
				*pos++=0x0A;
				...
				ofstream testsend;
   				testsend.open("/data/factorytestsend.txt", ios::app);
				for(int j = 0; j < 8; ++j)
				{
					testsend << hex << *(dataBuff + j) <<endl;
					FACTORYPATTERN("%02x ", *(dataBuff + j));
				}
				testsend.close();
...
uint8_t FactoryPattern_Communication::unpack_ChangeTboxMode(uint8_t *pData, int len)
{
	...
	//kill other thread
	pack_hande_data_and_send( MSG_COMMAND_ModeID, mCurrentTboxPattern);
...
uint8_t FactoryPattern_Communication::pack_hande_data_and_send(uint16_t MsgID, uint8_t state)
{
	...
	*pos++=0;
	*pos++=0;
	dataLen =  2;//pack_Protocol_Data(pos, BUFFER_SIZE, MsgID, state);	
