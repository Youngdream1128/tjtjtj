...
int main(int argc,char* argv[]) 
{
	...
	int  SRS_Voltage = 0;//检测SRS电压值
	int  SRSCheckPeriod = 0;//检测高电平的周期
	struct timeval tvBeginCheckSRS, tvPeriodCheckSRS;//数据上报时间间隔ms
 	gettimeofday(&tvBeginCheckSRS, NULL);
	gettimeofday(&tvPeriodCheckSRS, NULL);
	...
	/*	gettimeofday(&tvPeriodCheckSRS, NULL);
		long cost_tvtime = 0;
		cost_tvtime = (tvPeriodCheckSRS.tv_sec*1000 + tvPeriodCheckSRS.tv_usec/1000) - (tvBeginCheckSRS.tv_sec*1000 + tvBeginCheckSRS.tv_usec/1000);
		if(cost_tvtime == 40)
		{
			//检测SRS波形
			if(adc_voltage_check(&SRS_Voltage) == 0)
			{
				printf("Current SRS_Voltage:%d mv\n", SRS_Voltage);
				if(SRS_Voltage < 400)//低电平
				{
				}
				else(SRS_Voltage < 1800 && SRS_Voltage > 1200)//高电平
				{
					SRSCheckPeriod++;
					if(SRSCheckPeriod )
				}	
