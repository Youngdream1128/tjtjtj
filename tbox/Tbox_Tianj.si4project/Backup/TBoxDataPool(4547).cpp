...
int TBoxDataPool::initPara(void)
{
	...
	setPara(E_CALL_ID, (void *)("18565753539"), strlen("18565753539"));
	setPara(B_CALL_ID, (void *)("18565753539"), strlen("18565753539"));
...
int TBoxDataPool::getPara(TBoxDataId id, void *pPara, uint32_t size)
{
	switch (id)
	{
		...
		case WIFI_APMODE_USED_IP:
			...
			break;
		case IMEI_INFO:
			memcpy(pPara, TBoxPara.dataPara.imei, sizeof(TBoxPara.dataPara.imei));
			break;
		case SIM_ICCID_INFO:
			memcpy(pPara, TBoxPara.dataPara.iccid, sizeof(TBoxPara.dataPara.iccid));
			break;
		case CIMI_INFO:
...
/*****************************************************************************
* Function Name : setVIN
* Description   : 设置车辆信息VIN
* Input			: char *vin;
*                 int size
* Output        : None
* Return        : 0:success, -1:faild
...
* Date          : 2018.01.18
*****************************************************************************/
int TBoxDataPool::setVIN(char *vin, int size)
{
...
/*****************************************************************************
* Function Name : getVIN
* Description   : 获得车辆信息VIN
* Input			: char *vin;
*                 int size
* Output        : None
* Return        : readLen:vin data length
* Auther        : ygg
* Date          : 2018.01.18
*****************************************************************************/
int TBoxDataPool::getVIN(char *vin, int size)
{
	uint16_t readLen;
    int fd = open(VEHICLE_IDENTIFY_NUM_FILE, O_RDONLY, 0664);
    if (-1 == fd)
    {
        DEBUGLOG("can't get VIN, please set the VIN first!\n");
        return -1;
    }
	
    if ((readLen = read(fd, vin, size)) <= 0)
    {
        DEBUGLOG("read failed!\n");
		close(fd);
        return -1;
