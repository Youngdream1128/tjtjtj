...
//PayloadBuff 执行了加密后的数据
	switch (applicationID)
	{
		...
		case ACPApp_RemoteCtrlID://远程控制
			...
			ErrorCodeDeal(ElementBuff + DATA_TIMESTAMP_SIZE + DATA_AUTHTOKEN_SIZE + DATA_SOURCE_SIZE, mcuUart::m_mcuUart->RCtrlErrorCode);//错误码暂时为0
...
uint16_t CFAWACP::addPastRecordWheelState(uint8_t* pTemp)
{
	...
	for(int i = 0;i < 19; i++)
	{
		...
		{
			Decrypt_AES128Data(p_FAWACPInfo_Handle->SKey, (dataBuff + SIGNATURE_SIZE + DATA_SOFEOF_SIZE + DATA_HEADER_SIZE), (dataLen - SIGNATURE_SIZE - DATA_SOFEOF_SIZE*2 - DATA_HEADER_SIZE), PayloadBuff, AcpCryptFlag_IS);
...
uint16_t CFAWACP::RootKeyDataDeal(uint8_t *dataBuff)
{
  ...
  //采用新Rootkey加密新的Rootkey
	printf("============================pos - dataBuff is datalen == %d\n", (pos - dataBuff));
	tempLen = 0;
  tempLen = Encrypt_AES128Data(p_FAWACPInfo_Handle->New_RootKey, p_FAWACPInfo_Handle->New_RootKey, sizeof(p_FAWACPInfo_Handle->RootKey), pos, AcpCryptFlag_IS);
  pos += 32;
	printf("============================p_FAWACPInfo_Handle->New_RootKey is datalen == %d\n", tempLen);
...
uint16_t CFAWACP::UnpackData_AcpUpdateRootKey(uint8_t *PayloadBuff, uint16_t payload_size, uint8_t MsgType)
{
	...
	switch(MsgType)
	{
		case 2:
			...
			FAWACPLOG("Request Message Source -- %d\n",RequestSource);
			DataLen = 0;
			DataLen = Decrypt_AES128Data(p_FAWACPInfo_Handle->RootKey, pos, 32, p_FAWACPInfo_Handle->New_RootKey, AcpCryptFlag_IS);
			pos += 32;
			printf("========================p_FAWACPInfo_Handle->New_RootKey is datalen == %d\n", DataLen);
			for(int i = 0; i < DataLen; i++)
				printf("%02x ", p_FAWACPInfo_Handle->New_RootKey[i]);
			printf("\n\n\n");
